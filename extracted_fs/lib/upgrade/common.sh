...
run_ramfs() { # <command> [...]
	install_bin /bin/busybox /bin/ash /bin/sh /bin/mount /bin/umount	\
		...
		/bin/rm /usr/bin/basename /bin/kill /bin/chmod /usr/bin/expr
	...
	install_file /etc/fw_env.config /etc/resolv.conf /lib/*.sh /lib/functions/*.sh /lib/upgrade/*.sh $RAMFS_COPY_DATA
...
default_do_upgrade() {
	...
	if [ "$SAVE_CONFIG" -eq 1 ]; then
		get_image "$1" | mtd $MTD_CONFIG_ARGS -j "$CONF_TAR" write $ARGV "${PART_NAME:-image}"
	else
		get_image "$1" | mtd write $ARGV "${PART_NAME:-image}"
	fi
}

upgrade_check_error_led(){
	v "upgrade check error"
	[ -e /tmp/upgrade_ok ] && return 0
	error_no=`cat /tmp/upgrade_error`
	case "$error_no" in
		"1")
		on_delay=1
		off_delay=1	
